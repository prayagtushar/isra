"""Sector normalization. Over-merging is the worse failure: distinct sectors must stay apart."""
...
class TestSpellingVariants:
    ...
    def test_collapses_the_same_sector_named_at_three_lengths(self):
        """All three appeared in one corpus, so the filter listed three chips meaning one thing."""
...
class TestThingsThatMustStaySeparate:
    def test_keeps_financial_services_apart_from_financial_technology(self):
        """A bank and a payments startup are not the same sector."""
...
class TestOutputShape:
    ...
    def test_leaves_an_unknown_sector_alone_apart_from_casing(self):
        """The map is a fixup list, not an allowlist."""
