...
export function useChatStream() {
  const { active, activeId, newConversation, appendMessage } =
    useConversations();
  const { mode, topK, showRetrievalTrace } = useSettings();
  ...
  const traceRef = useRef<RetrievalTrace | undefined>(undefined);
  ...
  const send = useCallback(
    async (questionRaw: string) => {
      ...
      try {
        for await (const evt of streamChat(
          { question, history, top_k: topK, mode, trace: showRetrievalTrace },
          controller.signal,
        )) {
          if (evt.type === "sources") {
            ...
            traceRef.current = evt.trace;
            setStreaming((s) => (s ? { ...s, trace: evt.trace } : s));
    ...
    [active, activeId, appendMessage, mode, newConversation, topK, showRetrievalTrace, setStageStatus],
  );
