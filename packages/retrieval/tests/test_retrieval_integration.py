...
def test_keyword_search_long_query_returns_partial_matches(conn: Connection):
    ...
    startup_id = conn.execute(
        """
        INSERT INTO startups (normalized_name, source_url, name, description)
        VALUES ('partial-match-co', 'https://example.com/pm', 'Partial Match Co', 'A test startup')
        RETURNING id
        """
    ).fetchone()[0]
    chunk_id = conn.execute(
        """
        INSERT INTO chunks (startup_id, chunk_index, text, embedding)
        VALUES (%s, 0, 'integration test fintech company headquartered in Bangalore', NULL)
        RETURNING id
        """,
        (startup_id,),
    ).fetchone()[0]
    ...
    try:
        query = (
            "Indian fintech startups founded in Mumbai or Bangalore "
            "that have raised Series A or later funding"
        )
        results = search_keyword(conn, query, top_k=100)
        assert len(results) > 0
        assert chunk_id in [c.id for c in results]
    finally:
        conn.execute("DELETE FROM chunks WHERE id = %s", (chunk_id,))
...
def test_retrieve_hybrid_rerank_mode(
    ...
    monkeypatch.setattr(
        pipeline_mod,
        "rerank",
        lambda _query, chunks, top_k, rerank_top_k=20: [
            c for c in chunks if c.id == seeded_chunk.id
        ][:top_k],
    )
